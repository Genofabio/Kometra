...
public class ImageAnalysisService : IImageAnalysisService
{
    ...
    public Point FindCenterOfLocalRegion(Mat regionMat)
    {
        ...
        if (dynamicRange <= 1e-6) return new Point(workingMat.Width / 2.0, workingMat.Height / 2.0);
    ...
    public Point FindGaussianCenter(Mat rawMat, double sigma = 3.0)
    {
        ...
        try
        {
            ...
            if (double.IsNaN(result[1]) || double.IsNaN(result[2])) throw new Exception("NaN");
    ...
    public Point FindPeak(Mat rawMat, double sigma = 1.0)
    {
        ...
        if (y0 <= 0 || x0 <= 0 || y0 >= blurredMat.Rows - 1 || x0 >= blurredMat.Cols - 1) return new Point(x0, y0);
        try
        {
    ...
    // =======================================================================
    // 4. ANALISI SPOSTAMENTO STELLE (Phase Correlation Robusta)
    // =======================================================================

    public Point ComputeStarFieldShift(Mat reference, Mat target)
    {
        if (reference.Empty() || target.Empty() || reference.Width != target.Width) return new Point(0, 0);

        using var ref32 = new Mat();
        using var tgt32 = new Mat();
        reference.ConvertTo(ref32, MatType.CV_32FC1);
        target.ConvertTo(tgt32, MatType.CV_32FC1);
        Cv2.PatchNaNs(ref32, 0);
        Cv2.PatchNaNs(tgt32, 0);

        // Pre-Processing: Log -> Blur -> Laplacian
        using var refProc = ProcessForPhaseCorr(ref32);
        using var tgtProc = ProcessForPhaseCorr(tgt32);

        using var hann = new Mat();
        Cv2.CreateHanningWindow(hann, ref32.Size(), MatType.CV_32FC1);
        Point2d shift = Cv2.PhaseCorrelate(refProc, tgtProc, hann, out double response);

        if (response < 0.01) return new Point(0, 0);
        
        // Return negativo (-x, -y) perché AlignmentService somma questo valore 
        // a totalShift e poi lo SOTTRAE dal master.
        // Se PhaseCorrelate dice "sposta current di +10 per matchare prev",
        // significa che current è a SINISTRA.
        // In realtà PhaseCorrelate(Ref, Tgt) dice lo shift per sovrapporre Tgt su Ref.
        return new Point(-shift.X, -shift.Y);
    }

    // --- HELPER DI PRE-PROCESSING ---
    private Mat ProcessForPhaseCorr(Mat input32f)
    {
        using var tempLog = new Mat();
        Cv2.Add(input32f, new Scalar(1.0), tempLog);
        Cv2.Log(tempLog, tempLog); // Logaritmo (Uniforma intensità)

        using var tempBlur = new Mat();
        Cv2.GaussianBlur(tempLog, tempBlur, new OpenCvSharp.Size(5, 5), 1.5); // Blur (Via hot pixel)

        Mat resultLap = new Mat();
        Cv2.Laplacian(tempBlur, resultLap, MatType.CV_32FC1, ksize: 3); // Laplaciano (Via cometa)
        return resultLap;
    }
}