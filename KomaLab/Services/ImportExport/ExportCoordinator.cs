...
public class ExportCoordinator : IExportCoordinator
{
        ...
        CancellationToken token)
    {
        ...
        _metadataService.AddValue(commonHeader, "CREATOR", "KomaLab");
        ...
        for (int i = 0; i < sourceData.Count; i++)
        {
            ...
            _metadataService.AddValue(extHeader, "HISTORY", $"KomaLab - MEF Compression Mode: {settings.Compression}");
        ...
        for (int i = 0; i < items.Count; i++)
        {
            ...
            try
            {
                ...
                if (settings.Format == ExportFormat.Fits)
                {
                    ...
                    _metadataService.AddValue(exportHeader, "HISTORY", $"KomaLab - Compression Mode: {settings.Compression}");
                    _metadataService.SetValue(exportHeader, "CREATOR", "KomaLab", "Software name");
