...
using Kometra.Services.Processing.Rendering; // Necessario per IImagePresentationService
using OpenCvSharp;
        ...
        CancellationToken token = default)
    {
        Action<Mat, Mat, FitsHeader, int> posterizeOp = (src, dst, header, index) =>
        {
            ...
            _metadataService.AddValue(header, "HISTORY", 
                $"Kometra: Posterization ({levels} levels, {mode} scaling).");
            
            _metadataService.AddValue(header, "HISTORY", 
                $"Kometra: Applied thresholds - Black: {actualBlack:F1}, White: {actualWhite:F1} (Adaptive: {autoAdapt})");
